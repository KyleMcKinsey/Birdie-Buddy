...
# -------------------------------------------------------------
st.markdown("---")
st.subheader("2. Practice Asset Allocation & Focus Mode")

total_balls = st.number_input(
    "Total Practice Balls Available:",
    min_value=10,
    max_value=300,
    value=100,
    step=10,
...
if practice_mode == "Pure Grind Mode (100% Technical Drill)":
  grind_pct = 1.0
  st.caption("Pure Grind selected: All practice balls allocated to drill reps.")
elif practice_mode == "Pure Game Mode (100% Target Pressure)":
  grind_pct = 0.0
  st.caption(
      "Pure Game selected: All practice balls allocated to pressure course"
      " simulation."
  )
else:
...
with col_a:
  st.metric(
      "Grind Mode Balls", f"{grind_balls} balls", f"{int(grind_pct * 100)}%"
  )
with col_b:
